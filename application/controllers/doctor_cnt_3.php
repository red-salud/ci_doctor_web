...
/**
	...
	function guardaSiniestro(){
		 ...
		 /*$data['sin_tratamiento'] = $_POST['sin_tratamiento'];
		 $data['sin_laboratorio'] = $_POST['sin_laboratorio'];
		 $data['sin_diagnostico'] = $_POST['sin_diagnostico'];
	...
	function guardaSiniestroP2(){
		...
		if(empty($_POST['sin_diagnosticoSec'])){
		 ...
		 //datos para enviar a formulario 3

		 $data['sin_diagnosticoSec'] = $_POST['sin_diagnosticoSec'];			 

	//	 $data['sin_diagnosticoSec'] = $_POST['sin_diagnosticoSec'];
	//	 $data['sin_cantidadSec'] = nl2br($_POST['sin_cantidadSec']);
	//	 $data['sin_tratamientoSec'] = nl2br($_POST['sin_tratamientoSec']);
	//	 $data['sin_dosisSecundaria'] = nl2br($_POST['sin_dosisSecundaria']);
     ...
     if ($buscaAseg)
       {
         ...
         $nomEmp = $this->doctor_mdl->buscaEmp($codPlan);
            ...
            if ($nomEmp){
                       ...
                       $plan=$datos['plan'] = $nomEmp['plan_nom'];
       ;
                ...
                if($plan=='PLAN JUBILADO') { 
                    $edad=120;
                }
                else{ $edad=65;}
//         analiza la edad en el dia que fue registrado
        $analizarEdad=  $this-> CalculaEdadRegistro($buscaAseg['cert_iniVig'],$this->strtoDate($buscaAseg['aseg_fechNac']));
        if($analizarEdad < $edad){
       	$datos['analizarEdad'] = $analizarEdad;
       	...
       	if ($cancelado){
	    	...
	    	if ($fechCobrado){
		        ...
		        $diasTransDeCobro= $this->dias_transcurridos2($fechaIni, $fechaFin);
                        $datos['diastrans']=$diasTransDeCobro;
...
// Guardamos una OA y obtenemos el numero de OA

	//$this->load->model('doctor_mdl');
	//$buscaHC = $this->doctor_mdl->buscaHC($_POST['hcl_nomHC']);
		...
		//$data['numDoc'] = $_POST['numDoc'];
		 ...
		 /*$data['personales']=$buscaHC['hcl_antePers'];
		 $data['familiares']=$buscaHC['hcl_anteFam'];
		 $data['farmaceutico']=$buscaHC['hcl_alergiaFarm'];
		 $data['alimentos']=$buscaHC['hcl_alergiaAlim'];
		 $data['primConsulta']=$buscaHC['hcl_motPrimConsul'];*/
...
//$this->cargaAseguradoDNI($numDoc, $nomEst_Prov);
//$this->load->view('view_mcaBuscaRespuesta',$data);
...
function strtoDate($cadena){
	...
	function CalculaEdadRegistro( $fechaReg,$fecha ) {
		list($Y,$m,$d) = explode("-",$fecha);
		list($Y1,$m1,$d1) = explode("-",$fechaReg);
		return( $m1.$d1 < $m.$d ? $Y1-$Y-1 : $Y1-$Y );
}
